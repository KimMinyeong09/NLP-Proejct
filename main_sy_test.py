...
train_dataset = load_dataset("csv", data_files=f"dataset/parsed_data/{DNAME}/v2/train_special_token.csv")['train']
valid_dataset = load_dataset("csv", data_files=f"dataset/parsed_data/{DNAME}/v2/validation.csv")['train']
...
print("Special tokens:", tokenizer.all_special_tokens)  # ['<s>', '</s>', '<unk>', '<pad>', '<mask>']
...
for idx, batch in enumerate(train_loader): 
    global_input_ids = batch['global_input_ids']
    global_attention_mask = batch['global_attention_mask']
    global_emo = batch['global_emo']

    local_input_ids = batch['local_input_ids']
